...
Los criptoactivos declaran además `crypto_profile`: `bitcoin`, `ethereum`, `altcoin`, `stablecoin`
o `wrapped`. Los dos últimos se traducen a un perfil analítico `unsupported`; no heredan análisis de
Bitcoin. El catálogo productivo conserva únicamente Bitcoin. Los otros perfiles se prueban
sintéticamente para fijar el límite sin anunciar ingestión productiva.
