...
| `FUNDAMENTALS-COVERAGE` | `PLANNED` | Declaración de base de unidad de título (`security_unit_basis`) para emisores de acciones ordinarias integrada por #228; base ADR de `equity:us:bvn` y `equity:us:tsm` pendiente. Pasa a `PLANNED` por decisión humana explícita del 2026-09-12 ante la criticidad del almacenamiento físico. | Los **20** emisores SEC tienen corridas fundamentales exitosas y evidencia fundamental `present` en `universe-coverage-v1` con ventana de 3.660 días (Mesa desacopla ventana de consulta y frescura de 365 días: 20/20 presentes, BVN/TSM/MU overdue honestos). `FUNDAMENTALS-COVERAGE-1`/#228 (#229) quedó integrado en `b0c41cd`. |
| `DATA-CHASSIS` | `NEXT` | Chasis de datos, contención de almacenamiento, backup verificado y observabilidad física. Apertura de la ruta por `DATA-CHASSIS-0` (`ADVANCES`), instrumentación de la etapa 1 por `DATA-CHASSIS-1` (`ADVANCES`), cierre de la etapa 1 por `DATA-CHASSIS-2` (`ADVANCES`), cableado de la etapa 1 por `DATA-CHASSIS-3` (`ADVANCES`), apertura de la etapa 2 por `DATA-CHASSIS-4` (`ADVANCES`), adopción en pipelines por `DATA-CHASSIS-5` (`ADVANCES`), regla canónica de identidad de métrica v2 con adaptador de resolución por `DATA-CHASSIS-6` (`ADVANCES`), adaptador de lectura consciente de versión de identidad por `DATA-CHASSIS-7` (`ADVANCES`), adopción de la identidad v2 en el camino de escritura de mercado por `DATA-CHASSIS-8` (`ADVANCES`), adopción de la identidad v2 en el camino de escritura de derivados Deribit por `DATA-CHASSIS-9` (`ADVANCES`), apertura de la etapa 4 con el contrato aislado `EvidenceSet` por `DATA-CHASSIS-10` (`ADVANCES`), apertura de la etapa 6 con el journal acotado en el scheduler por `DATA-CHASSIS-13` (`ADVANCES`), corrección de defectos operacionales de identidad v2 por `DATA-CHASSIS-15` (`ADVANCES`), adopción del journal acotado en alertas operacionales y screening analítico por `DATA-CHASSIS-16` (`ADVANCES`) y extensión al almacén de crudo y posiciones 13F con ordenamiento determinista de historia 13F por `DATA-CHASSIS-17` (`ADVANCES`). | Etapa 0 entregada: backup verificado OPS-8 fuera del VHD, restore temporal probado, baseline en bytes exactos y registro de la ruta en `docs/data_chassis.md`. `DATA-CHASSIS-1` entrega la mitad medible de la etapa 1: contrato aditivo `storage-observability-v1`, colector read-only y hook mínimo del scheduler, sin cambiar el estado de ninguna fila. `DATA-CHASSIS-2` completa la etapa 1: clasificación del crecimiento por rol de tabla, overhead propio medido, ventanas de 7 y 30 días con ausencias declaradas, alerta de presupuesto operacional e informe read-only reproducible (`storage-observability-report-v1`) con CLI sin escritura. `DATA-CHASSIS-3` (#236) cablea el colector en la composición de producción (`scripts/serve_investment_analyst.py`). `DATA-CHASSIS-4` (#238) abrió la etapa 2 construyendo la capa de acceso y persistencia por lotes acotados (`get_many`, `save_many`, detección de conflictos en memoria y recibos tipados en `ObservationRepository`, `MetricResultRepository` y `DiagnosticResultRepository`) sin modificar un solo llamador. `DATA-CHASSIS-5` (#240) cerró el trabajo de código de la etapa 2 adoptando la API por lotes en los pipelines analíticos (`analytics/crypto/derivatives_pipeline.py` y `analytics/market/statistics_pipeline.py`), retirando el get posterior a save, memoizando el grafo de dependencias por corrida y limitando la verificación profunda a filas nuevas o en conflicto mediante `BatchWriteReceipt`. `DATA-CHASSIS-6` (#242) abre la etapa 3 definiendo la regla canónica de identidad de métrica v2 y su adaptador de resolución v1/v2 como contrato puro, aislado y disjunto sin llamador en producción. PLAN evaluó y rechazó extender `DiagnosticResult` por conflicto de contratos (valores agregados y pesos obligatorios ajenos a un corte descriptivo) y separó el snapshot por ausencia de mecanismo de migración en DuckDB (`SCHEMA_VERSION = 1` estricto sin runner). Por decisión humana explícita del 2026-09-16, el gate `−80 % real en jobs de reutilización` permanece reasignado a verificación post-despliegue mediante `scripts/report_storage_observability.py`. `DATA-CHASSIS-7` (#244) entrega la mitad de lectura de la etapa 3: los cinco caminos que hoy dependen de `parameters["known_at"]` de una métrica de mercado resuelven explícitamente la versión de identidad desde el `result_id` antes de aplicar su regla de corte, de modo que una fila v2 sin ese parámetro sea seleccionable de forma determinista y point-in-time y una fila v1 conserve su comportamiento; no emite una sola identidad v2 ni toca el motor, el pipeline, los módulos de identidad de mercado, el almacenamiento, el schema ni las dependencias. `DATA-CHASSIS-8` (#246) cruza la puerta de escritura de la etapa 3 para mercado: la familia de estadísticas emite identificadores UUID8 y deja de escribir `known_at` en `parameters`, de modo que un corte nuevo sin evidencia nueva reutiliza las métricas de ventana finita en vez de crear filas; el digest semántico del consolidado trata v1 y v2 de la misma coordenada como equivalentes; `metric_identity_v2` admite exactamente un llamador de producción declarado; y las cadenas recursivas EMA/RSI/ATR/MACD siguen creando filas por el desplazamiento diario de la ventana hasta la etapa 5. `DATA-CHASSIS-9` (#248) cierra la adopción de escritura de la etapa 3 para las dos familias que concentran el 99,75 % de las métricas: los derivados Deribit (`funding.sum_1h`, `funding.mean_1h`, `dvol.change_points`, `perpetual.bid_ask_spread_bps`) emiten identificadores UUID8 sin `known_at` en `parameters`, de modo que un corte nuevo sin evidencia nueva reutiliza cada métrica de la ventana programada, y el replay histórico del servicio read-only resuelve primero la fila v2, después la v1 del mismo corte y por último el candidato en memoria, conservando el `diagnostic_id` de las filas v1 persistidas; `metric_identity_v2` admite exactamente dos llamadores de producción declarados. La etapa 3 queda pendiente de la decisión viva entre el cierre con `AnalysisSnapshot` (separado por falta de runner de migración) y la apertura de la etapa 4 con el `EvidenceSet` de funding. `DATA-CHASSIS-10` (#250) abre la etapa 4 con la **representación** del lineage compartido como contrato puro, determinista y probado, **sin persistirlo y sin llamador**: segmentos inmutables de día UTC completo bajo la política versionada `evidence-segmentation-v1`, conjuntos que son recortes ordenados de esos segmentos con un desplazamiento de cabeza y `input_count`, hash canónico independiente de la segmentación e identidades UUID8 de dominio propio que excluyen `metric_key` —de modo que suma y media de la misma ventana comparten conjunto—, corte, reloj, valor y ventana; la verificación recalcula segmentos y hash y falla cerrado. Por decisión humana del 2026-09-18 no persiste nada: los dos candidatos que dejó abiertos `DATA-CHASSIS-9` exigen almacenamiento nuevo y no existe runner de migración, así que la adopción persistida queda condicionada al workspace v2 de la etapa 7 y este bloque no reduce bytes reales todavía. |
| `CRYPTO-DATA-DISCOVERY` | `PLANNED` | Ampliación del universo cripto spot con fuente oficial y contrato PIT propio por familia de datos. Depende de `DATA-CHASSIS`. | `crypto:btc-usd` y `crypto:eth-usd` ya operan sobre Coinbase; el resto exige fuente, licencia y retención declaradas antes de cualquier contrato. Promoción canónica desde el Issue #210; no autoriza BUILD. |
| `ASSET-EXPANSION` | `PLANNED` | Alta de nuevas identidades de catálogo con fuente oficial autorizada y cobertura verificable. Depende de `DATA-CHASSIS`. | Cada alta exige entrada de catálogo, binding de proveedor, `source_id` y evidencia PIT; el catálogo es superficie protegida. Promoción canónica desde el Issue #210; no autoriza BUILD. |

La transición integrada registra `DATA-CHASSIS-0`/#230 (#231) sobre `DATA-CHASSIS` (`ADVANCES`), integrado en `85971de`: apertura de la ruta transversal de chasis de datos y almacenamiento con contención operacional (backup verificado OPS-8, restore temporal y baseline en bytes exactos sobre copia restaurada), `FUNDAMENTALS-COVERAGE` en `PLANNED` por decisión humana explícita del 2026-09-12 ante la criticidad del almacenamiento físico y `DATA-CHASSIS` como único `NEXT`. `DATA-CHASSIS-1`/#232 (#233) quedó integrado en `468c6d1`: entregó la instrumentación de observabilidad de almacenamiento (contrato aditivo `storage-observability-v1`, colector read-only y hook mínimo del scheduler) sin cambiar el estado de ninguna fila.
La transición de `DATA-CHASSIS-2`/#234 quedó integrada en `06cbeee`: completó la etapa 1 del chasis entregando su mitad legible —clasificación del crecimiento por rol de tabla y overhead propio medidos por el colector, ventanas de 7 y 30 días con ausencias declaradas, alerta de presupuesto operacional e informe read-only reproducible ejecutable contra el `state_root` del writer o de una copia restaurada— sin cambiar el estado de ninguna fila.
La transición de `DATA-CHASSIS-3`/#236 (#237) quedó integrada en `9a5dff9`: activó en producción el instrumento que `DATA-CHASSIS-1` construyó y que `DATA-CHASSIS-2` hizo legible, cableando `StorageObservabilityCollector` en `scripts/serve_investment_analyst.py` con el `state_root` y el `database_path` que la composición ya resuelve, sin tocar el contrato del scheduler, la CLI pública, el colector, el informe ni una sola línea de `src/investment_analyst/`.
La transición de `DATA-CHASSIS-5`/#240 (#241) quedó integrada en `d555411`: cerró el trabajo de código de la etapa 2 adoptando la API por lotes en los pipelines analíticos (`analytics/crypto/derivatives_pipeline.py` y `analytics/market/statistics_pipeline.py`), retirando el get posterior a save, memoizando el grafo de dependencias por corrida y limitando la verificación profunda a filas nuevas o en conflicto mediante `BatchWriteReceipt`. La verificación del gate `−80 % real en jobs de reutilización` de la etapa 2 permanece pendiente post-despliegue mediante `scripts/report_storage_observability.py`.
Este candidato (`DATA-CHASSIS-6`/#242) propone route_effect: ADVANCES sobre `DATA-CHASSIS`: abre la etapa 3 entregando la regla canónica de identidad semántica v2 y su adaptador de resolución v1/v2 como contrato aislado y probado sin llamadores. Registra la evaluación y rechazo de la extensión de `DiagnosticResult` y la separación de `AnalysisSnapshot` por ausencia de mecanismo de migración en el almacenamiento actual.
Transición de ruta propuesta: route_transition:DATA-CHASSIS:ADVANCES.
Este candidato (`DATA-CHASSIS-7`/#244) propone route_effect: ADVANCES sobre `DATA-CHASSIS`: entrega
el adaptador de lectura consciente de versión de la etapa 3 —la regla aislada de elegibilidad de corte
que resuelve v1/v2 desde el `result_id` y los cinco caminos de lectura que hoy dependen de
`parameters["known_at"]` de una métrica de mercado— sin emitir una sola identidad v2. PLAN particiona
la adopción exactamente como ya particionó las etapas 1 y 2: la lectura aprende primero y
`DATA-CHASSIS-8` cruza después la puerta de escritura, porque dos de los cinco puntos
(`alerts/analytical_monitor.py:64` y `alerts/analytical_backtest.py:316,358`) fallan **en silencio**
con una fila v2 —descartan la métrica sin error, apagan la alerta de mercado y dejan el backtest sin
cortes— y ese defecto no es detectable por CI verde ni por revisión del bloque de escritura.
Transición de ruta propuesta: route_transition:DATA-CHASSIS:ADVANCES.
Este candidato (`DATA-CHASSIS-8`/#246) propone route_effect: ADVANCES sobre `DATA-CHASSIS`: cruza la
puerta de un solo sentido de la etapa 3 en mercado —`statistics_identity` gana el punto de entrada v2 y
el motor emite UUID8 sin `known_at` en `parameters`, mientras el pipeline comprueba versión de identidad
y PIT en vez del parámetro legado— y normaliza el sexto lector que #244 no enumeró: el digest semántico
del consolidado, que sin esta corrección rompería la vista consolidada de cada activo de mercado el día
de la transición. Ninguna fila v1 se reasigna, reescribe ni borra, y el llamador único de
`metric_identity_v2` queda declarado y probado. Registra el límite vigente: las cadenas recursivas
EMA/RSI/ATR/MACD siguen creando filas por el desplazamiento diario de la ventana hasta la etapa 5.
Transición de ruta propuesta: route_transition:DATA-CHASSIS:ADVANCES.
Este candidato (`DATA-CHASSIS-9`/#248) propone route_effect: ADVANCES sobre `DATA-CHASSIS`: adopta la
identidad v2 en el camino de escritura de derivados Deribit —las cuatro familias emiten UUID8 y dejan de
escribir `known_at` en `parameters`, de modo que un corte nuevo sin evidencia nueva reutiliza la ventana
programada en vez de volver a materializarla— y añade el único punto que #247 no nombró: el replay
histórico del servicio read-only resuelve primero la fila v2, después la v1 del mismo corte y por último
el candidato en memoria, conservando el `diagnostic_id` de las filas v1 persistidas. Con esto la
adopción de escritura cubre el 99,75 % de las métricas de `metric_results`; la etapa 3 queda pendiente de
la decisión viva entre cerrarla con `AnalysisSnapshot` (separado por falta de runner de migración) y
abrir la etapa 4 con el `EvidenceSet` de funding.
Transición de ruta propuesta: route_transition:DATA-CHASSIS:ADVANCES.
Este candidato (`DATA-CHASSIS-10`/#250) propone route_effect: ADVANCES sobre `DATA-CHASSIS`: abre la
etapa 4 con el contrato aislado del lineage compartido —`EvidenceSegment` por día UTC completo bajo la
política versionada `evidence-segmentation-v1` y `EvidenceSet` como recorte ordenado de segmentos con
desplazamiento de cabeza e `input_count`, hash canónico independiente de la segmentación e identidades
UUID8 de dominio propio— sin persistir nada y sin llamador, y demuestra la equivalencia con el motor
real: cada métrica de funding resuelve exactamente a su `input_observation_ids` con su mismo
`available_at`, suma y media comparten conjunto y el lineage segmentado ocupa ≤ 10 % de los bytes de las
listas repetidas en una fixture de 60 días. Registra la decisión humana del 2026-09-18 y su desviación:
la etapa preveía ventanas «almacenadas una vez», pero la adopción persistida queda condicionada al
...
Transición de ruta propuesta: route_transition:DATA-CHASSIS:ADVANCES.
Este candidato (`DATA-CHASSIS-15`/#259) propone route_effect: ADVANCES sobre `DATA-CHASSIS`: corrige los dos defectos operacionales del primer despliegue con identidad v2 (release `7fe76a6`) que impedían operar el scheduler y provocaban fallo de memoria en BTC/ETH. Reemplaza el listado por activo y rango por lectura directa por identificador (`get_many`) en `statistics_pipeline` y `derivatives_pipeline` evitando lecturas de otras familias métricas, implementa la política versionada `analytical-window-revision-selection-v1` para la selección determinista de revisiones v2 dependientes de la ventana en `AnalyticalMetricSnapshotSelector` y `AnalyticalBacktestService`, y aisla los intentos con error analítico registrándolos como `SKIPPED` en `AnalyticalScreeningMonitor` para evitar que aborten la reconciliación de arranque del servicio.
Transición de ruta propuesta: route_transition:DATA-CHASSIS:ADVANCES.
Este candidato (`DATA-CHASSIS-16`/#261) propone route_effect: ADVANCES sobre `DATA-CHASSIS`: cierra el trabajo de código de la etapa 6 adoptando el journal acotado (`bounded-operational-journal-v1`) en `OperationalAlertStateStore` y `AnalyticalScreeningStateStore`. Reemplaza la reescritura monolítica en O(n) por appends en O(1) al open segment en registros y transiciones sin reserializar la historia, incorpora caché en memoria en `AnalyticalScreeningStateStore` para responder consultas de deduplicación (`contains_attempt`) en O(1) con cero I/O de disco y cero decodificaciones JSON repetidas, preserva el archivo v1 existente byte a byte plegándolo una sola vez a snapshot inicial, y garantiza recuperación determinista ante líneas parciales incompletas.
Transición de ruta propuesta: route_transition:DATA-CHASSIS:ADVANCES.
Este candidato (`DATA-CHASSIS-17`/#265) propone route_effect: ADVANCES sobre `DATA-CHASSIS`: extiende la persistencia por lotes de la etapa 2 al almacén de registros crudos (`JsonRawRecordRepository.save_many`) y la adopta en la ingesta de posiciones 13F (`InstitutionalHoldingsRepository.save_positions`), acotando las consultas de verificación de existencia a un único SELECT por lote de hasta 1.000 registros e inserción multi-fila atómica en vez de SELECT e INSERT individuales por posición. Adicionalmente corrige el defecto operacional de ordenamiento en `institutional_metric_engine.py` donde valores `None` en `put_call` provocaban `TypeError: '<' not supported between instances of 'str' and 'NoneType'`, asegurando que la historia de posiciones 13F se ordene de forma determinista y sin fallos.
Transición de ruta propuesta: route_transition:DATA-CHASSIS:ADVANCES.
...
- `frontend/local_web.py` tiene 3.045 líneas y reúne protocolos, controlador, aplicación web,
...
- La respuesta `/api/overview` incluye el detalle completo de 103 trabajos y ocupa casi 100 KiB,
...
- los 103 trabajos se ejecutan secuencialmente y con horarios próximos. Falta un presupuesto
