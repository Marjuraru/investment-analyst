...
| `DATA-CHASSIS` | `NEXT` | Chasis de datos, contención de almacenamiento, backup verificado y observabilidad física. Apertura de la ruta por `DATA-CHASSIS-0` (`ADVANCES`). | Etapa 0 entregada: backup verificado OPS-8 fuera del VHD, restore temporal probado, baseline en bytes exactos y registro de la ruta en `docs/data_chassis.md`. Siguiente bloque: `DATA-CHASSIS-1`. |
...
La transición integrada registra `FUNDAMENTALS-COVERAGE-1`/#228 (#229) sobre `FUNDAMENTALS-COVERAGE` (`ADVANCES`), que quedó integrado en `b0c41cd`. Este candidato (`DATA-CHASSIS-0`/#230) propone route_effect: ADVANCES sobre `DATA-CHASSIS`: abre la ruta transversal de chasis de datos y almacenamiento con contención operacional (backup verificado OPS-8, restore temporal y baseline en bytes exactos sobre copia restaurada), dejando a `DATA-CHASSIS` como único `NEXT` y pasando `FUNDAMENTALS-COVERAGE` a `PLANNED` por decisión humana explícita del 2026-09-12 ante la criticidad del almacenamiento físico.
