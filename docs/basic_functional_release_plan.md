...
| `DATA-CHASSIS` | `NEXT` | Chasis de datos, contención de almacenamiento, backup verificado y observabilidad física. Apertura de la ruta por `DATA-CHASSIS-0` (`ADVANCES`), instrumentación de la etapa 1 por `DATA-CHASSIS-1` (`ADVANCES`), cierre de la etapa 1 por `DATA-CHASSIS-2` (`ADVANCES`) y cableado de la etapa 1 por `DATA-CHASSIS-3` (`ADVANCES`). | Etapa 0 entregada: backup verificado OPS-8 fuera del VHD, restore temporal probado, baseline en bytes exactos y registro de la ruta en `docs/data_chassis.md`. `DATA-CHASSIS-1` entrega la mitad medible de la etapa 1: contrato aditivo `storage-observability-v1`, colector read-only y hook mínimo del scheduler, sin cambiar el estado de ninguna fila. `DATA-CHASSIS-2` completa la etapa 1: clasificación del crecimiento por rol de tabla, overhead propio medido, ventanas de 7 y 30 días con ausencias declaradas, alerta de presupuesto operacional e informe read-only reproducible (`storage-observability-report-v1`) con CLI sin escritura. La etapa 2 no está entregada. `DATA-CHASSIS-3` (#236) cablea el colector en la composición de producción (`scripts/serve_investment_analyst.py`), de modo que cada job ejecutado por el scheduler deja su registro bajo el `state_root` resuelto; la etapa 2 pasa a `DATA-CHASSIS-4`. Siguiente bloque: `DATA-CHASSIS-4`. |
...
Este candidato (`DATA-CHASSIS-2`/#234) propone route_effect: ADVANCES sobre `DATA-CHASSIS`: completa la etapa 1 del chasis entregando su mitad legible — clasificación del crecimiento por rol de tabla y overhead propio medidos por el colector, ventanas de 7 y 30 días con las ausencias declaradas de forma explícita, alerta de presupuesto operacional contra un umbral configurable e informe read-only reproducible ejecutable contra el `state_root` del writer o de una copia restaurada —, sin cambiar el estado de ninguna fila, sin completar la ruta y dejando `DATA-CHASSIS-3` (etapa 2, persistencia y verificación por lotes) como siguiente bloque.
Este candidato (`DATA-CHASSIS-3`/#236) propone route_effect: ADVANCES sobre `DATA-CHASSIS`: activa en producción el instrumento que `DATA-CHASSIS-1` construyó y que `DATA-CHASSIS-2` hizo legible, cableando `StorageObservabilityCollector` en `scripts/serve_investment_analyst.py` con el `state_root` y el `database_path` que la composición ya resuelve, sin tocar el contrato del scheduler, la CLI pública, el colector, el informe ni una sola línea de `src/investment_analyst/`. No completa la ruta y desplaza la etapa 2 a `DATA-CHASSIS-4`: el gate `−80 % real en jobs de reutilización` de esa etapa exige una línea base medida **antes** del cambio de persistencia, y hoy no existe ninguna porque la composición no inyectaba el colector; publicar la etapa 2 sin este cableado declararía un criterio no satisfacible. Desplegar el runtime sigue siendo una acción operativa del humano y no es criterio de este bloque: el cableado es lo que hace que ese despliegue empiece a producir telemetría.
