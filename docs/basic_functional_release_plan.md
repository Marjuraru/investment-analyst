...
| `SEC-CORPUS` | `NEXT` | Correspondencia 13F PIT, features y reglas descriptivas, capas 2/3 de observaciones y métricas, y capa 4 de eventos/candidatos persistidos para insiders y 13D/13G integrados; faltan panel, cobertura 13F generalizada y ejecución autónoma. | Las posiciones 13F conservan CUSIP y clase declarados sin `asset_id` y quedan fuera de esta capa 4; el percentil robusto 13F permanece `not_evaluable` hasta que se conecte su serie histórica. |
