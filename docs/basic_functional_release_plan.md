...
| `SEC-CORPUS` | `NEXT` | Correspondencia 13F PIT, features descriptivas read-only para 13F, insiders y 13D/13G, y motor de reglas descriptivas por formulario (SEC-CORPUS-8) integrados; falta la capa de evento/candidato persistida y trazable, y el panel. | Las posiciones 13F conservan CUSIP y clase declarados sin `asset_id`; el percentil robusto 13F queda declarado en el catálogo pero permanece `not_evaluable` hasta que se conecte su serie histórica. |
