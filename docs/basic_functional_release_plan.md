...
| `SEC-CORPUS` | `NEXT` | #134/#135 integraron correspondencia y observaciones 13F; #140 integró composición/completitud, #142 métricas de cambio reportado, #144 peso declarado y #146 concentración efímera. #151 añade eventos institucionales descriptivos persistidos (`cazatiburones_institutional_events_v1`), candidatos con cooldown de 24h y consulta reproducible por snapshot_id; #153 añade notificación local deduplicada y reanudable de candidatos elegibles de ambas verticales; #155 añade línea temporal y búsqueda local point-in-time (`sec-document-timeline-v1`) para ambas familias documentales; #159 añade camino de lectura HTTP local de solo lectura para la línea temporal SEC y Cazatiburones por activo (`/api/v1/sec-document-timeline`, `/api/v1/cazatiburones/declared-activity`, `/api/v1/cazatiburones/institutional-observations`) sin modificar contratos ni almacenamiento. Pendientes: cartera efectiva, reglas complejas y UI. | Las observaciones 13F son filas as-filed separadas; peso, concentración, eventos y notificaciones usan el universo as-filed del cierre efectivo, nunca una cartera efectiva. La outbox local, la línea temporal y los endpoints de lectura HTTP conservan familias separadas y no son señal, recomendación ni canal externo. No hay percentil robusto ni capa 4. |
