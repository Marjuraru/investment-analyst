...
| `FUNDAMENTALS-COVERAGE` | `PLANNED` | Cobertura fundamental multi-activo más allá del emisor Apple; exige declaración de frontera de capacidad antes de BUILD. | Hoy sólo `equity:us:aapl` recorre el pipeline fundamental completo (Submissions y Company Facts). Promoción canónica desde el Issue #210; no autoriza BUILD y pasa a único `NEXT` sólo cuando `EQUITY-UNIVERSE` cierre. |
...
La transicion integrada registra RUNTIME-EFFICIENCY-5/#223 integrado sobre `EQUITY-UNIVERSE`, que conserva su
unico `NEXT`; este candidato (RUNTIME-EFFICIENCY-6) propone `route_effect: ADVANCES` sobre el mismo item y
registra `RUNTIME-EFFICIENCY-7` como siguiente bloque real dentro de la fila. `FUNDAMENTALS-COVERAGE`,
`CRYPTO-DATA-DISCOVERY` y `ASSET-EXPANSION` se promueven como filas `PLANNED`; BVL sigue `BLOCKED` y
PREDICTIVE-RESEARCH sigue `DEFERRED`. Ninguna promocion autoriza BUILD.
Este candidato propone route_effect: ADVANCES sobre `EQUITY-UNIVERSE`; el item no se cierra y sigue existiendo
exactamente un `NEXT`.
Transicion de ruta propuesta: route_transition:EQUITY-UNIVERSE:ADVANCES.
