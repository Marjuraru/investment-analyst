...
| `DATA-CHASSIS` | `NEXT` | Chasis de datos, contención de almacenamiento, backup verificado y observabilidad física. Apertura de la ruta por `DATA-CHASSIS-0` (`ADVANCES`) e instrumentación de la etapa 1 por `DATA-CHASSIS-1` (`ADVANCES`). | Etapa 0 entregada: backup verificado OPS-8 fuera del VHD, restore temporal probado, baseline en bytes exactos y registro de la ruta en `docs/data_chassis.md`. `DATA-CHASSIS-1` entrega la mitad medible de la etapa 1: contrato aditivo `storage-observability-v1`, colector read-only y hook mínimo del scheduler, sin cambiar el estado de ninguna fila. Siguiente bloque: `DATA-CHASSIS-2`. |
...
La transición integrada registra `DATA-CHASSIS-0`/#230 (#231) sobre `DATA-CHASSIS` (`ADVANCES`), integrado en `85971de`: apertura de la ruta transversal de chasis de datos y almacenamiento con contención operacional (backup verificado OPS-8, restore temporal y baseline en bytes exactos sobre copia restaurada), `FUNDAMENTALS-COVERAGE` en `PLANNED` por decisión humana explícita del 2026-09-12 ante la criticidad del almacenamiento físico y `DATA-CHASSIS` como único `NEXT`. Este candidato (`DATA-CHASSIS-1`/#232) propone route_effect: ADVANCES sobre `DATA-CHASSIS`: entrega la instrumentación de observabilidad de almacenamiento de la etapa 1 (contrato aditivo `storage-observability-v1`, colector read-only y hook mínimo del scheduler) sin cambiar el estado de ninguna fila y sin completar la etapa ni la ruta.
