...
| `EQUITY-UNIVERSE` | `NEXT` | Capacidad multi-activo integrada en 37 identidades del catálogo y 20 jobs de scheduler; pendiente retirar los residuos de privilegio por ticker en `RUNTIME-EFFICIENCY-5`. | Capacidad multi-activo integrada para 37 identidades. `UNIVERSE-COVERAGE-2`/#169 añade el camino de lectura HTTP local de solo lectura que faltaba para la matriz de cobertura completa (`GET /api/v1/universe-coverage`), reutilizando literalmente `universe-coverage-v1` (sin nuevo contrato, DTO ni `schema_version`) más su cacheo acotado e invalidación; no toca la interfaz. Inventario verificado de residuos de activo implícito en el runtime: `src/investment_analyst/frontend/local_web.py:1997` (`or "equity:us:aapl"`), `src/investment_analyst/frontend/local_web.py:2351` (`or "crypto:btc-usd"`, no listado antes), `src/investment_analyst/frontend/local_web.py:2481` (`or APPLE_ASSET_ID`), los cuatro defaults de firma `asset_id: str = APPLE_ASSET_ID` en `src/investment_analyst/frontend/local_web.py:1403,1426,1449,1474`, `src/investment_analyst/application/aapl_refresh_planner.py:37-39` (`ASSET_ID`, `SOURCE_ID` y `"AAPL"`, no sólo el símbolo) y `src/investment_analyst/providers/market/alpaca_stock.py:15` (`SUPPORTED_SYMBOL = "AAPL"` sin uso), todos en `RUNTIME-EFFICIENCY-5`; `local_web.py:1632` ya no apunta a ningún fallback. Los dos contratos Apple (`src/investment_analyst/analytics/market/chart_models.py:568` `aapl-market-chart-v5` y `src/investment_analyst/application/operational_models.py:86` `AaplDailyRunRequestSnapshot`) son identidad de contrato legado, no privilegio implícito: su retirada queda como Work Block posterior separado. |
...
La transicion integrada registra LOCAL-INTERFACE DONE por UI-14/#202 (route_effect: COMPLETES); SEC-CORPUS completa su ruta con este candidato (route_effect: COMPLETES), EQUITY-UNIVERSE pasa a unico NEXT con `RUNTIME-EFFICIENCY-5` como siguiente bloque, BVL sigue BLOCKED y PREDICTIVE-RESEARCH sigue DEFERRED.
Este candidato propone route_effect: COMPLETES sobre SEC-CORPUS; el item pasa a DONE y deja exactamente un NEXT en `EQUITY-UNIVERSE`.
Transicion de ruta propuesta: route_transition:SEC-CORPUS:COMPLETES.
