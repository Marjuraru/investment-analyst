...
| `LOCAL-INTERFACE` | `PLANNED` | `UI-1` integró el sistema visual verificable (contrato estático de tokens, contraste y gramática de ausencia); `UI-2` sustituye la página única acumulativa por el armazón de seis tableros (`mesa`, `activo`, `tecnico`, `revisar`, `cazatiburones`, `sistema`) sin perder capacidad entregada; `UI-3` converge el lenguaje visual interior de esas secciones al lienzo aprobado (paleta cálida R≥G≥B, reglas de 1 px en vez de sombra/esquina redondeada, densidad de fila del lienzo) sin mover una sola sección; `UI-4` conecta el tablero `cazatiburones` a los tres caminos de lectura SEC/13F ya integrados por `SEC-CORPUS` (línea temporal documental, actividad declarada y observaciones institucionales 13F), sin tocar transporte, contratos ni almacenamiento. | Fila creada por `UI-2` (`route_effect: ADVANCES`); `UI-3` y `UI-4` avanzan la misma fila, ambas `ADVANCES`. Permanece `PLANNED` y ningún candidato completa nada. `UI-4` conecta `cazatiburones` (deja de ser `not-built`) a `/api/v1/sec-document-timeline`, `/api/v1/cazatiburones/declared-activity` y `/api/v1/cazatiburones/institutional-observations`, reasignado de `UI-3` a `UI-4` por decisión humana explícita del 2026-09-05 —converger el lenguaje primero evitó construir el tablero nuevo en el lenguaje visual que `UI-3` sustituyó—. Pendiente: el punto 5.4 de esta sección (dividir JavaScript y CSS por componentes) y la vendorización de IBM Plex Sans/Mono, ambas deuda explícita y no ejecutada aquí. |
| `SEC-CORPUS` | `NEXT` | #134/#135 integraron correspondencia y observaciones 13F; #140 integró composición/completitud, #142 métricas de cambio reportado, #144 peso declarado y #146 concentración efímera. #171 añade la proyección efímera `sec-13f-effective-public-holdings-policy-v1`: `RESTATEMENT` sustituye y `NEW HOLDINGS ENTRIES` posterior suplementa, sin persistir cartera. #151 añade eventos institucionales descriptivos persistidos (`cazatiburones_institutional_events_v1`), candidatos con cooldown de 24h y consulta reproducible por snapshot_id; #153 añade notificación local deduplicada y reanudable de candidatos elegibles de ambas verticales; #155 añade línea temporal y búsqueda local point-in-time (`sec-document-timeline-v1`) para ambas familias documentales; #159 añade camino de lectura HTTP local de solo lectura para la línea temporal SEC y Cazatiburones por activo (`/api/v1/sec-document-timeline`, `/api/v1/cazatiburones/declared-activity`, `/api/v1/cazatiburones/institutional-observations`) sin modificar contratos ni almacenamiento. Pendientes: reglas complejas y UI. | Las observaciones 13F son filas as-filed separadas; la proyección efectiva es read-only y sólo expone contribuyentes completos y trazables. Peso, concentración, eventos y notificaciones usan el universo as-filed del cierre efectivo, nunca una cartera persistida. La outbox local, la línea temporal y los endpoints de lectura HTTP conservan familias separadas y no son señal, recomendación ni canal externo. No hay percentil robusto ni capa 4. |
