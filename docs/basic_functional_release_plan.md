...
| `FUNDAMENTALS-COVERAGE` | `NEXT` | Declaración de base de unidad de título (`security_unit_basis`) para emisores de acciones ordinarias y gramática de frescura/frecuencia en cobertura; base ADR pendiente. | Los **20** emisores SEC tienen corridas fundamentales exitosas y evidencia fundamental `present` en `universe-coverage-v1` con ventana de 3.660 días (Mesa desacopla ventana de consulta y frescura de 365 días: 20/20 presentes, BVN/TSM/MU overdue honestos). `FUNDAMENTALS-COVERAGE-1`/#228 avanza este ítem (`route_effect: ADVANCES`): declara la base de título de 16 acciones ordinarias (`reported_common_share`, factor `"1"`), eliminando `share_basis_unavailable` en su valoración corporativa, mientras `equity:us:bvn` y `equity:us:tsm` permanecen pendientes como ADRs con `share_basis_unavailable`. `FUNDAMENTALS-COVERAGE` conserva su estado `NEXT`. |
...
La transición integrada registra `RUNTIME-EFFICIENCY-7`/#227 sobre `EQUITY-UNIVERSE` (`COMPLETES`), que dejó a
`FUNDAMENTALS-COVERAGE` como único `NEXT`. Este candidato (`FUNDAMENTALS-COVERAGE-1`/#228) propone
route_effect: ADVANCES sobre `FUNDAMENTALS-COVERAGE`: declara la base de título de las 16 acciones ordinarias y
la ventana fundamental de Mesa (3.660 días) desacoplada de la frescura (365 días), dejando la base ADR de
`equity:us:bvn` y `equity:us:tsm` como resto declarado; `FUNDAMENTALS-COVERAGE` conserva su estado `NEXT`.
Transición de ruta propuesta: route_transition:FUNDAMENTALS-COVERAGE:ADVANCES.
