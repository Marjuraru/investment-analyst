...
| `SEC-CORPUS` | `NEXT` | Correspondencia 13F PIT, features descriptivas read-only para 13F, insiders y 13D/13G, motor de reglas descriptivas por formulario (SEC-CORPUS-8) y capa 2 de observaciones normalizadas point-in-time para insiders y 13D/13G (SEC-CORPUS-9) integrados; faltan las métricas descriptivas persistidas (capa 3), la capa de evento/candidato persistida y trazable, y el panel. | Las posiciones 13F conservan CUSIP y clase declarados sin `asset_id` y quedan fuera de la capa 2; el percentil robusto 13F queda declarado en el catálogo pero permanece `not_evaluable` hasta que se conecte su serie histórica. |
