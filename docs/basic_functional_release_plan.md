...
| `SEC-CORPUS` | `NEXT` | Correspondencia 13F PIT, features descriptivas read-only para 13F, insiders y 13D/13G, motor de reglas descriptivas por formulario (SEC-CORPUS-8), capa 2 de observaciones normalizadas point-in-time para insiders y 13D/13G (SEC-CORPUS-9) y capa 3 de métricas descriptivas persistidas para el delta de tenencia de insiders y el delta de propiedad beneficiaria 13D/13G (SEC-CORPUS-10) integrados; falta la capa de evento/candidato persistida y trazable, y el panel. | Las posiciones 13F conservan CUSIP y clase declarados sin `asset_id` y quedan fuera de la capa 2 y de la capa 3; el percentil robusto 13F queda declarado en el catálogo pero permanece `not_evaluable` hasta que se conecte su serie histórica. |
