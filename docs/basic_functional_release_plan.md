...
| `EQUITY-UNIVERSE` | `PLANNED` | Capacidad multi-activo integrada en 37 identidades del catálogo y 20 jobs de scheduler; pendiente retirar los residuos de privilegio por ticker en `RUNTIME-EFFICIENCY-1`. | Capacidad multi-activo integrada para 37 identidades. `UNIVERSE-COVERAGE-2`/#169 añade el camino de lectura HTTP local de solo lectura que faltaba para la matriz de cobertura completa (`GET /api/v1/universe-coverage`), reutilizando literalmente `universe-coverage-v1` (sin nuevo contrato, DTO ni `schema_version`) más su cacheo acotado e invalidación; no toca la interfaz. Pendiente resolver cuatro residuos de ticker: `src/investment_analyst/frontend/local_web.py:1632` (fallback a Apple si se omite `asset_id`), `src/investment_analyst/application/aapl_refresh_planner.py:39` (símbolo `"AAPL"` por defecto), `src/investment_analyst/analytics/market/chart_models.py:568` y `src/investment_analyst/application/operational_models.py:86` (`asset_id: Literal["equity:us:aapl"]` estructural), y `src/investment_analyst/providers/market/alpaca_stock.py:15` (`SUPPORTED_SYMBOL = "AAPL"` sin uso), todos en `RUNTIME-EFFICIENCY-1`. |
| `LOCAL-INTERFACE` | `PLANNED` | `UI-1` integró el sistema visual verificable (contrato estático de tokens, contraste y gramática de ausencia); `UI-2` sustituye la página única acumulativa por el armazón de seis tableros (`mesa`, `activo`, `tecnico`, `revisar`, `cazatiburones`, `sistema`) sin perder capacidad entregada; `UI-3` converge el lenguaje visual interior de esas secciones al lienzo aprobado (paleta cálida R≥G≥B, reglas de 1 px en vez de sombra/esquina redondeada, densidad de fila del lienzo) sin mover una sola sección; `UI-4` conecta el tablero `cazatiburones` a los tres caminos de lectura SEC/13F ya integrados por `SEC-CORPUS` (línea temporal documental, actividad declarada y observaciones institucionales 13F), sin tocar transporte, contratos ni almacenamiento; `UI-5` mueve las cargas del arranque a una matriz por activación, conserva una sola marca de `known_at` y descarta respuestas superadas por activo/corte; `UI-6` reorganiza `mesa` en la jerarquía de lectura del lienzo (novedades desde el corte anterior en tres familias separadas, en qué confío, qué está roto, universo al final) y la convierte en el primer consumidor de `universe-coverage-v1`, sin crear, renombrar ni eliminar ningún tablero. | Fila creada por `UI-2` (`route_effect: ADVANCES`); `UI-3`, `UI-4`, `UI-5` y `UI-6` avanzan la misma fila, todas `ADVANCES`. Permanece `PLANNED` y ningún candidato completa nada. `UI-4` conecta `cazatiburones` (deja de ser `not-built`) a `/api/v1/sec-document-timeline`, `/api/v1/cazatiburones/declared-activity` y `/api/v1/cazatiburones/institutional-observations`, reasignado de `UI-3` a `UI-4` por decisión humana explícita del 2026-09-05 —converger el lenguaje primero evitó construir el tablero nuevo en el lenguaje visual que `UI-3` sustituyó—. `UI-5` deja documentada la matriz tablero→peticiones→estados y la invalidación en memoria; no modifica transporte ni superficie visual. `UI-6` conecta `mesa` a `/api/v1/candidate-notifications` y `/api/alerts` (ya integrados, primera lectura desde este tablero) y a `/api/v1/universe-coverage` (integrado por `UNIVERSE-COVERAGE-2` sin tocar la interfaz; `UI-6` es su primer consumidor); institucional 13F y actividad declarada quedan declaradas sin camino de lectura universo-wide -- exponerlas por HTTP es un Work Block de `BUILD_PRODUCT` posterior, no ejecutado aquí. Pendiente: el punto 5.4 de esta sección (dividir JavaScript y CSS por componentes), la vendorización de IBM Plex Sans/Mono, y la convergencia de `activo`, `tecnico`, `revisar` y `cazatiburones` a esta misma jerarquía, todas deuda explícita y no ejecutada aquí. |
| `SEC-CORPUS` | `NEXT` | #134/#135 integraron correspondencia y observaciones 13F; #140 integró composición/completitud, #142 métricas de cambio reportado, #144 peso declarado y #146 concentración efímera. #171 añade el cierre público efectivo 13F read-only con composición v2 (`sec-13f-composition-policy-v2` y `sec-13f-effective-public-holdings-policy-v1`): `RESTATEMENT` sustituye y `NEW HOLDINGS` posterior suplementa, sin persistir cartera. #151 añade eventos institucionales descriptivos persistidos (`cazatiburones_institutional_events_v1`), candidatos con cooldown de 24h y consulta reproducible por snapshot_id; #153 añade notificación local deduplicada y reanudable de candidatos elegibles de ambas verticales; #155 añade línea temporal y búsqueda local point-in-time (`sec-document-timeline-v1`) para ambas familias documentales; #159 añade camino de lectura HTTP local de solo lectura para la línea temporal SEC y Cazatiburones por activo (`/api/v1/sec-document-timeline`, `/api/v1/cazatiburones/declared-activity`, `/api/v1/cazatiburones/institutional-observations`) sin modificar contratos ni almacenamiento. Pendientes: reglas complejas. | Las observaciones 13F son filas as-filed separadas; la proyección efectiva es read-only y sólo expone contribuyentes completos y trazables. Peso, concentración, eventos y notificaciones usan el universo as-filed del cierre efectivo, nunca una cartera persistida. La outbox local, la línea temporal y los endpoints de lectura HTTP conservan familias separadas y no son señal, recomendación ni canal externo. No hay percentil robusto ni capa 4. #184 añade `cazatiburones-universe-activity-v1` y `GET /api/v1/cazatiburones/universe-activity`: presencia, ausencia y frescura as-filed de `insider`, `beneficial` e `institutional` por activo, sin resolver reportes ni crear agregados. #186 añade `GET /api/v1/cazatiburones/notifications` como lectura HTTP local read-only de la outbox `cazatiburones-notification-v1`, con familias, acuses persistidos y truncación declarada, sin modificar contratos ni almacenamiento. |
...
La transición integrada en la tabla conserva `EQUITY-UNIVERSE` | `PLANNED`,
crea `LOCAL-INTERFACE` | `PLANNED` y mantiene `SEC-CORPUS` | `NEXT` como
único candidato elegible; ningún ítem pasa a `DONE`.
