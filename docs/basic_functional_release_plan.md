...
| `DATA-CHASSIS` | `NEXT` | Chasis de datos, contención de almacenamiento, backup verificado y observabilidad física. Apertura de la ruta por `DATA-CHASSIS-0` (`ADVANCES`), instrumentación de la etapa 1 por `DATA-CHASSIS-1` (`ADVANCES`), cierre de la etapa 1 por `DATA-CHASSIS-2` (`ADVANCES`), cableado de la etapa 1 por `DATA-CHASSIS-3` (`ADVANCES`) y apertura de la etapa 2 por `DATA-CHASSIS-4` (`ADVANCES`). | Etapa 0 entregada: backup verificado OPS-8 fuera del VHD, restore temporal probado, baseline en bytes exactos y registro de la ruta en `docs/data_chassis.md`. `DATA-CHASSIS-1` entrega la mitad medible de la etapa 1: contrato aditivo `storage-observability-v1`, colector read-only y hook mínimo del scheduler, sin cambiar el estado de ninguna fila. `DATA-CHASSIS-2` completa la etapa 1: clasificación del crecimiento por rol de tabla, overhead propio medido, ventanas de 7 y 30 días con ausencias declaradas, alerta de presupuesto operacional e informe read-only reproducible (`storage-observability-report-v1`) con CLI sin escritura. `DATA-CHASSIS-3` (#236) cablea el colector en la composición de producción (`scripts/serve_investment_analyst.py`). `DATA-CHASSIS-4` (#238) abre la etapa 2 construyendo la capa de acceso y persistencia por lotes acotados (`get_many`, `save_many`, detección de conflictos en memoria y recibos tipados en `ObservationRepository`, `MetricResultRepository` y `DiagnosticResultRepository`) sin modificar un solo llamador. La etapa 2 se particiona en dos bloques (`DATA-CHASSIS-4` capacidad pura aditiva y `DATA-CHASSIS-5` adopción en pipelines y migración de dobles de test). Por decisión humana explícita del 2026-09-16, el gate `−80 % real en jobs de reutilización` se reasigna a verificación post-despliegue mediante `scripts/report_storage_observability.py`. Siguiente bloque: `DATA-CHASSIS-5`. |
...
Este candidato (`DATA-CHASSIS-4`/#238) propone route_effect: ADVANCES sobre `DATA-CHASSIS`: abre la etapa 2 de la ruta entregando la capa de acceso y persistencia por lotes acotados en almacenamiento (`get_many`, `save_many`, detección de conflictos en memoria y recibos tipados de escritura que distinguen filas creadas, reutilizadas y en conflicto) sobre `ObservationRepository`, `MetricResultRepository` y `DiagnosticResultRepository`, sin modificar ningún llamador ni doble de test y dejando `analytics/**` intacto. Registra la partición de la etapa 2 en dos bloques (`DATA-CHASSIS-4` y `DATA-CHASSIS-5`) y la reasignación del gate de `−80 % real en jobs de reutilización` a verificación post-despliegue con `scripts/report_storage_observability.py` por decisión humana explícita del 2026-09-16, dejando `DATA-CHASSIS-5` como siguiente bloque.
