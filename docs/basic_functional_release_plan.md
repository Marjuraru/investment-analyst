...
| `LOCAL-INTERFACE` | `PLANNED` | `UI-1` integró el sistema visual verificable (contrato estático de tokens, contraste y gramática de ausencia); `UI-2` sustituye la página única acumulativa por el armazón de seis tableros (`mesa`, `activo`, `tecnico`, `revisar`, `cazatiburones`, `sistema`) sin perder capacidad entregada. | Fila creada por `UI-2` (`route_effect: ADVANCES`) porque la interfaz no tenía representación en la ruta; permanece `PLANNED` y no completa nada. Pendiente: conectar el tablero `cazatiburones` (reservado `not-built`) en `UI-3`, y el punto 5.4 de esta sección (dividir JavaScript y CSS por componentes), declarado como deuda explícita y no ejecutado aquí. |
