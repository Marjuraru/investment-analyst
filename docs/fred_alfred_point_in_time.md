...
La composición del cliente, el almacenamiento y la consulta pasa por
`InvestmentAnalystApplication`: `refresh_fred_vintage(...)` abre un único writer y
`query_fred_point_in_time(...)` abre el workspace en solo lectura. Los scripts documentados abajo
son adaptadores de esa fachada y no crean rutas arquitectónicas paralelas.
...
Para el servicio persistente, guarda `FRED_API_KEY` en el `.env` privado usado por la unidad
`systemd`. Si la variable falta o está vacía, la interfaz continúa funcionando y simplemente no
registra trabajos macro automáticos.
...
## Catálogo y automatización acotada

El catálogo `fred-series-catalog-v1` organiza series independientes por dominio. La primera
automatización admite `GDPC1`, `CPIAUCSL`, `UNRATE`, `FEDFUNDS`, `TOTALSL` y `M2SL`. En el primer
refresh de cada serie descubre el vintage oficial más reciente e importa una instantánea completa.
Después consulta únicamente el borde posterior al último vintage almacenado, procesa como máximo
tres revisiones por ejecución y deja cobertura incompleta visible cuando debe reanudar otro lote.

`DCOILWTICO`, `DTWEXBGS` y `T10Y2Y` permanecen en el catálogo con automatización desactivada: sus
frecuencias diarias requieren primero Parquet columnar, particionado y backfill por ventanas. El
backfill de vintages anteriores al primer snapshot también queda separado de la actualización
diaria para que décadas de revisiones no bloqueen el servicio normal.

Todavía no se incluyen metadatos oficiales persistidos de serie, normalización macro columnar,
panel macro, relaciones con activos, diagnósticos de régimen, interpretación causal, predicción ni
recomendación.
