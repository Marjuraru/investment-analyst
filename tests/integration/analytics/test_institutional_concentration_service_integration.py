...
<tableEntryTotal>2</tableEntryTotal><tableValueTotal>100.00</tableValueTotal>
</edgarSubmission>"""
_TABLE = b"""<informationTable>
<infoTable><nameOfIssuer>APPLE INC</nameOfIssuer><titleOfClass>COM</titleOfClass>
<cusip>037833100</cusip><value>75.00</value><shrsOrPrnAmt><sshPrnamt>10</sshPrnamt>
<sshPrnamtType>SH</sshPrnamtType></shrsOrPrnAmt></infoTable>
<infoTable><nameOfIssuer>MICROSOFT CORP</nameOfIssuer><titleOfClass>COM</titleOfClass>
<cusip>594918104</cusip><value>25.00</value><shrsOrPrnAmt><sshPrnamt>10</sshPrnamt>
<sshPrnamtType>SH</sshPrnamtType></shrsOrPrnAmt></infoTable>
</informationTable>"""
...
def test_read_only_application_and_cli_return_same_declared_concentration(tmp_path: Path) -> None:
    ...
    result = CazatiburonesInstitutionalConcentrationApplication.create_default().query(
        manager_cik="1067983", known_at=_NOW, location=location
    )

    assert len(result) == 1
    assert result[0].status == "calculated"
    assert result[0].largest_declared_weight == Decimal("0.75")
    assert result[0].top_five_declared_weight is None
    with LocalStorage(paths, read_only=True) as storage:
        assert storage.raw_records.count() == raw_before

    script = Path(__file__).parents[3] / "scripts" / "query_sec_institutional_concentration.py"
    ...
    assert from_cli[0]["policy_version"] == "sec-13f-concentration-policy-v1"
    assert from_cli[0]["effective_artifact_id"] == str(result[0].effective_artifact_id)
