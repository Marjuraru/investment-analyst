#!/usr/bin/env python3
"""Consulta/replay read-only de derivados BTC/ETH por corte point-in-time local."""
...
def main() -> int:
    """Query only persisted evidence and print a compact versioned result."""
    arguments = _parser().parse_args()
    try:
        result = InvestmentAnalystApplication.create_default().query_crypto_derivatives(
            CryptoDerivativesQueryRequest(
        ...
        print(
            json.dumps(
                {
                    "notice": (
                        "Diagnóstico multidimensional descriptivo; no score, señal, "
                        "recomendación, asesoramiento ni ejecución."
                    ),
        ...
        print(f"Crypto derivatives query failed: {error}", file=sys.stderr)
        return 1
