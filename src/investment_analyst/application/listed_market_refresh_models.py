"""Strict contracts for one catalog-backed Alpaca listed-asset refresh."""
...
class ListedMarketRefreshRequest(ContractModel):
    """Bounded market-only request for one Alpaca catalog asset."""
    ...
    def validate_range(self) -> "ListedMarketRefreshRequest":
        """Require one non-empty inclusive calendar range."""
        if self.market_start > self.market_end:
            raise ValueError("market_start must not be later than market_end")
        return self


class ListedMarketRefreshSummary(ContractModel):
    """Auditable outcome of Alpaca ingestion and independent market analytics."""

    model_config = ConfigDict(extra="forbid", frozen=True)

    schema_version: Literal["listed-market-refresh-v1"] = "listed-market-refresh-v1"
    asset_id: NonEmptyStr
    ...
    coverage_receipts_created: int = Field(ge=0)
    ...
    @classmethod
    def require_traceability(cls, value: object) -> object:
        """Require a real verified boolean."""
        if not isinstance(value, bool):
            raise ValueError("traceability_verified must be a bool")
        return value

    @model_validator(mode="after")
    def validate_summary(self) -> "ListedMarketRefreshSummary":
        """Keep asset, plan, range, cut, and execution counters aligned."""
    ...
    def to_json_dict(self) -> dict[str, object]:
        """Return explicit JSON primitives without credentials or raw payloads."""
