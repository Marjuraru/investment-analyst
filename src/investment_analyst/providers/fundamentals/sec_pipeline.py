"""Persist configurable SEC EDGAR foundation documents as auditable raw snapshots."""

from dataclasses import dataclass
from datetime import datetime
from uuid import UUID

from investment_analyst.core.models import Asset, AssetClass, RawRecord
...
class SecIssuerImportSummary:
    """Compact result of one two-document SEC issuer snapshot import."""
...
SecAaplImportSummary = SecIssuerImportSummary


class SecIssuerFundamentalsPipeline:
    """Fetch and persist one issuer's raw SEC documents without analytics."""
    ...
    def run(self) -> SecIssuerImportSummary:
        ...
        target_asset = create_sec_asset(self._configuration, existing_asset)
        source_definitions = get_sec_source_definitions(self._configuration)
        ...
        return SecIssuerImportSummary(
            asset_id=self._configuration.asset_id,
            cik=self._configuration.cik,
    ...
    def _prepare_candidates(
        self,
        fetch: SecIssuerFetchResult,
    ) -> dict[SecDocumentType, RawRecord]:
        if fetch.cik != self._configuration.cik or fetch.ticker != self._configuration.ticker:
            raise StorageError("SEC fetch result does not match the configured issuer")
        candidates: dict[SecDocumentType, RawRecord] = {}
        for document in fetch.documents:
            candidate = sec_document_to_raw_record(document, self._configuration)
            if candidate.record_id != expected_record_id(document, self._configuration):
    ...
    def _validate_candidate(self, record: RawRecord, document: SecEdgarDocument) -> None:
        expected_source = (
            self._configuration.submissions_source_id
            if document.document_type is SecDocumentType.SUBMISSIONS
            else self._configuration.companyfacts_source_id
        )
        if (
    ...
    def _verify_stored_record(self, record: RawRecord, document: SecEdgarDocument) -> None:
        if record.record_id != expected_record_id(document, self._configuration):
            raise StorageError("stored SEC raw record has an unexpected identifier")
        if record.asset_id != self._configuration.asset_id:
    ...
    def _verify_round_trip(
        ...
        if self._storage.assets.get(self._configuration.asset_id) != asset:
            raise StorageError("SEC issuer asset round-trip verification failed")
        for source in get_sec_source_definitions(self._configuration):
...
SecAaplFundamentalsPipeline = SecIssuerFundamentalsPipeline
